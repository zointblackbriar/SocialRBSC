...
abstract contract ComponentRole is Component, ERC165Query {
    ...
    function setCore(address _coreAddress) external {
        ...
        core = ComponentCore(_coreAddress);
    }

     /// @dev Sets the `_compartmentAddresss` to `role`.
    function setCompartment(address _compartmentAddresss) external {
        if (_compartmentAddresss != address(0)) {
            bool isCompartment = doesContractImplementInterface(
                _compartmentAddresss,
                InterfaceIds.COMPARTMENT_ID
            );
            require(isCompartment, "Doesn't support compartment interface.");
        }
        assignedCompartment = Compartment(_compartmentAddresss);
    }

     /// @dev Modifier that checks  `componentCore` has been set and
     /// the core's `compartment` matches this role's `compartment`.
    ...
    function addRole(bytes32 spec, address role) external override {
        //if the same role has been added, it should be thought of the different version of the role
    ...
    function removeRole(bytes32 spec) external override virtual {
        core.removeRole(spec);
    }

     /// @dev Returns `true` if `component` has been granted role with `spec`. - delegation
    ...
    function getRole(bytes32 spec) external override virtual returns (address) {
        return core.getRole(spec);
    }

     /// @dev Sets a `_compartment` to the `component` to indicate entering a context.
     /// @notice If another `_compartment` has already been set, it gets overriden.
    function activateCompartment(address _compartment) external override {
        core.activateCompartment(_compartment);
    }

     /// @dev Unsets the `_compartment` if there is one activated. - delegation
    function deactivateCompartment() external override {
        core.deactivateCompartment();
    }

     /// @dev Returns the `address` of the activated `compartment`. - delegation
